...
@typeassert(user_pos_dict=dict, user_rating_dict=dict, len_seqs=int, len_next=int, pad=(int, None))
def _generative_time_order_positive_items(user_pos_dict, user_rating_dict, len_seqs=1, len_next=1, pad=None):
    if not user_pos_dict:
        raise ValueError("'user_pos_dict' cannot be empty.")

    users_list, item_seqs_list, next_items_list = [], [], []
    rating_seqs_list, next_ratings_list = [], []
    user_n_pos = OrderedDict()
    ...
    for user, seq_items in user_pos_dict.items():
        ...
        seq_ratings = user_rating_dict[user]
        if isinstance(seq_ratings, np.ndarray):
            seq_ratings = np.array(seq_ratings, dtype=np.float32)
        if len(seq_items) >= tot_len:
            ...
            for idx in range(len(seq_items)-tot_len+1):
                ...
                tmp_ratings = seq_ratings[idx:idx+tot_len]
                rating_seqs_list.append(tmp_ratings[:len_seqs].reshape([1, len_seqs]))
                next_ratings_list.append(tmp_ratings[len_seqs:].reshape([1, len_next]))
            ...
            next_ratings_list.append(seq_ratings[-len_next:].reshape([1, len_next]))
            tmp_rating_seqs = pad_sequences([seq_ratings[:-len_next]], value=pad, max_len=len_seqs,
                                     padding='pre', truncating='pre', dtype=np.float32)
            rating_seqs_list.append(tmp_rating_seqs.squeeze().reshape([1, len_seqs]))
    ...
    rating_seqs_arr = np.concatenate(rating_seqs_list).squeeze()
    next_ratings_arr = np.concatenate(next_ratings_list).squeeze()

    return user_n_pos, users_arr, item_seqs_arr, next_items_arr, rating_seqs_arr, next_ratings_arr
...
class TimeOrderPairwiseSampler(Sampler):
    ...
    def __init__(self, dataset, len_seqs=1, len_next=1, pad=None, num_neg=1,
        ...
        self.len_next = len_next
        self.user_pos_dict = dataset.to_user_dict(by_time=True)
        self.user_rating_dict = dataset.to_user_dict_rating(by_time=True)

        self.user_n_pos, self.all_users, self.all_item_seqs, self.pos_next_items, self.all_rating_seqs, self.next_ratings = \
            _generative_time_order_positive_items(self.user_pos_dict, self.user_rating_dict, len_seqs=len_seqs,
                                                  len_next=len_next, pad=pad)
    ...
    def __iter__(self):
        ...
        data_iter = DataIterator(self.all_users, self.all_item_seqs, self.pos_next_items, neg_next_items, self.all_rating_seqs, self.next_ratings,
                                 batch_size=self.batch_size, shuffle=self.shuffle, drop_last=self.drop_last)

        for bat_users, bat_item_seqs, bat_pos_items, bat_neg_items, bat_rating_seqs, bat_next_rating in data_iter:
            yield np.asarray(bat_users), np.asarray(bat_item_seqs), \
                  np.asarray(bat_pos_items), np.asarray(bat_neg_items), \
                  np.asarray(bat_rating_seqs), np.asarray(bat_next_rating)
