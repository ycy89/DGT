...
class Trainer(AbstractTrainer):
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        def rating_pred_loss_level(pre_rating, target_rating):
            ...
            all_result = {'mse':mse_result, 'mae':mae_result, 'macro_mse':macro_mse,
                          'macro_mae':macro_mae, 'weighted_mse':weighted_mse, 'weighted_mae':weighted_mae}
            return weighted_mse, all_result
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                for batch_idx, batched_data in enumerate(iter_data):
                    score = self.model.predict_mse(batched_data[0].to(self.device))
                    scores.extend(score.tolist())
                    labels.extend(batched_data[0]['rating'].tolist())


                # self.model.fast_predict_mse(valid_data)
                # valid_score, valid_result = rating_pred_loss_level()
                valid_score, valid_result = rating_pred_loss_level(scores, labels)
...
class DSSRecTrainer(PretrainTrainer):
    r"""S3RecTrainer is designed for S3Rec, which is a self-supervised learning based sequential recommenders.
        It includes two training stages: pre-training ang fine-tuning.
