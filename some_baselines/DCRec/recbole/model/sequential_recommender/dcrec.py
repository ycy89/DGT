...
def graph_dual_neighbor_readout(g: dgl.DGLGraph, aug_g: dgl.DGLGraph, node_ids, features):
    ...
    assert not features.sum().isnan(), 'input_features_nan'
    _, all_neighbors = g.out_edges(node_ids)
    all_nbr_num = g.out_degrees(node_ids)

    # logging.info(f'all_neighbors:{all_neighbors}')
    # logging.info(f'all_neighbors.shape:{all_neighbors.shape}')
    # logging.info(f'all_nbr_num:{all_nbr_num}')
    # logging.info(f'all_nbr_num.shape:{all_nbr_num.shape}')
    # logging.info(f'zero_num_of_all_nbr_num:{(all_nbr_num == 0).sum()}')
    _, foreign_neighbors = aug_g.out_edges(node_ids)
    for_nbr_num = aug_g.out_degrees(node_ids)
    # logging.info(f'for_nbr_num:{for_nbr_num}')
    # logging.info(f'for_nbr_num.shape:{for_nbr_num.shape}')
    # logging.info(f'zero_num_of_for_nbr_num:{(for_nbr_num == 0).sum()}')

    # logging.info(f'foreign_neighbors:{foreign_neighbors}')
    # logging.info(f'foreign_neighbors.shape:{foreign_neighbors.shape}')
    # logging.info(f'for_nbr_num:{for_nbr_num}')
    # logging.info(f'for_nbr_num.shape:{for_nbr_num.shape}')
    ...
    # logging.info(f'foreign_neighbors:{foreign_neighbors}')
    # logging.info(f'for_lens:{for_lens}')
    for_feats = features[foreign_neighbors].split(for_lens)
    # logging.info(f'for_feats.shape:{for_feats.shape}')
    # for t in for_feats:
    #     assert not t.sum().isnan(), f'for_feat_nan:{t}'
    #     logging.info(f't.shape:{t.shape}')
    #     assert not t.mean(dim=0).sum().isnan(), f'for_feat_mean_nan:{t}'
...
def graph_augment(g: dgl.DGLGraph, user_ids, user_edges):
    ...
    # add edge if degree == 0
    edge_dict = {}
    add_edge_a, add_edge_b = [], []
    del_edge_a, del_edge_b = node_indicies_a.view(-1), node_indicies_b.view(-1)
    node_set = torch.cat([del_edge_a, del_edge_b], dim=0)
    for a, b in zip(del_edge_a.tolist(), del_edge_b.tolist()):
        edge_dict[a] = b
        edge_dict[b] = a
    node_set = node_set.unique()
    degrees = aug_g.out_degrees(node_set)
    for degree, node in zip(degrees.tolist(), node_set.tolist()):
        if degree == 0:
            add_edge_a.append(node)
            add_edge_b.append(edge_dict[node])
            add_edge_a.append(edge_dict[node])
            add_edge_b.append(node)
    aug_g.add_edge(torch.tensor(add_edge_a).to(torch.int64).to(g.device),
                   torch.tensor(add_edge_b).to(torch.int64).to(g.device))
    return aug_g
...
class DCRec(SequentialRecommender):
    ...
    def _subgraph_agreement(self, aug_g, raw_output_all, raw_output_seq, valid_items_flatten):
        ...
        # logging.info(f'aug_g:{aug_g}')
        # logging.info(f'valid_items_flatten:{valid_items_flatten}')
        # logging.info(f'raw_output_all:{raw_output_all}')
        s = ''
        if civil_nbr_ro.sum().isnan():
            s += 'civil_nbr_ro '
        if foreign_nbr_ro.sum().isnan():
            s += 'foreign_nbr_ro '
        if raw_output_seq.sum().isnan():
            s += 'raw_output_seq'
        if aug_output_seq.sum().isnan():
            s += 'aug_output_seq'
        if len(s) > 0:
            logging.info(s)
            assert len(s) == 0, s
        ...
        assert not agreement.sum().isnan()
    ...
    def calculate_loss_graphcl(self, interaction):
        ...
        pos_rating = interaction['rating']
        rating_seq = interaction['rating_list']
        ...
        # [item_num, H]
        # permute = torch.randint(self.n_items, [pos_items.shape[0]]).to(self.device)
        # test_item_emb = torch.cat([self.item_embedding.weight[pos_items].unsqueeze(1),
        #                            self.item_embedding.weight[permute].unsqueeze(1)], dim=1)
        test_item_emb = self.item_embedding.weight[pos_items]
        # logits = torch.matmul(seq_output, test_item_emb.transpose(0, 1))
        logits = self.MLP(torch.cat([seq_output, test_item_emb], dim=-1)).squeeze()
        # loss = self.loss_fct(logits+1e-8, pos_items)
        # target_logits = torch.zeros_like(logits)
        target_logits = pos_rating.to(logits.device)
        # for i in range(logits.shape[0]):
        #     target_logits[i][item_seq[i][:item_seq_len[i]]] = rating_seq[i][:item_seq_len[i]]
        #     target_logits[i][pos_items[i]] = pos_rating[i]
        loss = self.loss_fct(logits, target_logits)
    ...
    def predict_mse(self, interaction):
        item_seq = interaction[self.ITEM_SEQ]
        item_seq_len = interaction[self.ITEM_SEQ_LEN]
        # test_item = interaction["item_id_with_negs"]
        ...
        test_item_emb = self.item_embedding(target_item)  # [B, num, H]
        # scores = torch.matmul(seq_output.unsqueeze(
        #     1), test_item_emb.transpose(1, 2)).squeeze()
        scores = self.MLP(torch.cat([seq_output, test_item_emb], dim=-1)).squeeze()
        return scores