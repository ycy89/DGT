...
def sequential_augmentation(dataset):
    ...
    for i in range(len(old_data)):
        ...
        rating_seq = old_data['rating_list'][i]
        ...
        new_data['rating_list'].append(rating_seq)
        new_data['rating'].append(old_data['rating'][i].item())
        seq = seq[:seq_len]
        rating_seq = rating_seq[:seq_len]
        for end_point in range(1, seq_len):
            new_seq = seq[:end_point]
            new_rating_seq = rating_seq[:end_point]
            new_truth = seq[end_point].item()
            new_truth_rating = rating_seq[end_point].item()
            new_seq_len = len(new_seq)
            new_seq = torch.cat((new_seq, torch.zeros(max_item_list_len - new_seq_len, dtype=torch.long)), dim=0)
            new_rating_seq = torch.cat((new_rating_seq, torch.zeros(max_item_list_len - new_seq_len, dtype=torch.float32)), dim=0)
            ...
            new_data['rating_list'].append(new_rating_seq)
            new_data['rating'].append(new_truth_rating)
    ...
    new_data['rating_list'] = torch.stack(new_data['rating_list'], dim=0)
    new_data['rating'] = torch.tensor(new_data['rating'], dtype=torch.float32)
...
def get_args():
    ...
    parser.add_argument('--dataset', '-d', type=str, default='amazon_beauty', help='Benchmarks for session-based rec.')
...
if __name__ == '__main__':
    ...
    if args.dataset == "reddit":
    ...
    elif args.dataset == "beauty" or args.dataset == "sports" or args.dataset == "ml-20m" or args.dataset.startswith('amazon'):
        config_dict["graphcl_coefficient"] = 1e-5
        config_dict["graph_dropout_prob"] = 0.5
        config_dict["hidden_dropout_prob"]= 0.5
        config_dict["attn_dropout_prob"]  = 0.5
        config_dict["kl_weight"] = 1e-4
        config_dict['train_batch_size'] = 512

        if args.dataset == "beauty":# or 'beauty' in args.dataset:
        ...
        elif args.dataset == "sports" or args.dataset.startswith('amazon'):
    ...
    try:
        ...
        model = get_model(config['model'])(config, test_data.dataset, external_data).to(config['device'])
        logger.info(model)
