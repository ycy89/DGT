"""
From: https://github.com/hungthanhpham94/GRU4REC-pytorch
"""
import sys
...
class GRU4Rec(nn.Module):
    ...
    def create_final_activation(self, final_act):
        if final_act == 'tanh':
            ...
            print("Unknown final activation")
            sys.exit(1)

    def forward(self, input_, hidden):
        """
        Args:
            :param input_: (B,): a batch of item indices from a session-parallel mini-batch.
            :param hidden:
        Returns:
        ...
        if self.embedding_dim == -1:
            embedded = self.onehot_encode(input_)
            if self.training and self.dropout_input > 0:
                embedded = self.embedding_dropout(embedded)
            embedded = embedded.unsqueeze(0)
        else:
            embedded = input_.unsqueeze(0)
    ...
    def onehot_encode(self, input_):
        """
        Returns a one-hot vector corresponding to the input
        Args:
            input_ (B,): torch.LongTensor of item indices
        Returns:
            one_hot (B,C): torch.FloatTensor of one-hot vectors
        """
        self.onehot_buffer.zero_()
        index = input_.view(-1, 1)
        one_hot = self.onehot_buffer.scatter_(1, index, 1)
        return one_hot

    def embedding_dropout(self, input_):
        p_drop = torch.Tensor(input_.size(0), 1).fill_(1 - self.dropout_input)
        mask = torch.bernoulli(p_drop).expand_as(input_) / (1 - self.dropout_input)
