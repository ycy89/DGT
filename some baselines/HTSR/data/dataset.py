...
class Interaction(object):
    ...
    def to_user_dict_rating(self, by_time=False):
        if self._data.empty:
            warnings.warn("self._data is empty.")
            return None
        ...
        # read from buffer
        if by_time is True and "rate_user_dict_byt" in self._buffer:
            return deepcopy(self._buffer["rate_user_dict_byt"])
        if by_time is False and "rate_user_dict" in self._buffer:
            return deepcopy(self._buffer["rate_user_dict"])
        ...
        for user, user_data in user_grouped:
            if by_time:
                user_data = user_data.sort_values(by=[_TIME])
            user_dict[user] = user_data[_RATING].to_numpy(dtype=np.float32)

        # write to buffer
        if by_time is True:
            self._buffer["rate_user_dict_byt"] = deepcopy(user_dict)
        else:
            self._buffer["rate_user_dict"] = deepcopy(user_dict)
        return user_dict
...
class Dataset(object):
    ...
    def _load_data(self, data_dir, sep, columns):
        ...
        if os.path.isfile(train_file):
            _train_data = pd.read_csv(train_file, sep=sep)
        else:
            raise FileNotFoundError("%s does not exist." % train_file)

        valid_file = file_prefix + ".valid"
        if os.path.isfile(valid_file):
            _valid_data = pd.read_csv(valid_file, sep=sep)
        else:
        ...
        if os.path.isfile(test_file):
            _test_data = pd.read_csv(test_file, sep=sep)
        else:
        ...
        self.num_users = max(max(max(_train_data[_USER]), max(_test_data[_USER])), max(_valid_data[_USER])) + 1
        self.num_items = max(max(max(_train_data[_ITEM]), max(_test_data[_ITEM])), max(_valid_data[_ITEM])) + 1
        self.num_ratings = len(_train_data) + len(_valid_data) + len(_test_data)
        ...
        for i in range(len(_test_data)):
            self.test_data.append([_test_data[_USER][i], _test_data[_ITEM][i], _test_data[_RATING][i], _test_data['last_item'][i]])
